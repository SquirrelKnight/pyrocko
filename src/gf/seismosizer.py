...
class DoublePDR(SourceWithDerivedMagnitude):
    ...
    def discretize_basesource(self, store, target=None):
        ...
        return meta.DiscretizedMTSource(
            ...
            lon=self.lon,
            north_shifts=num.concatenate(
                (dss[0].north_shifts, dss[1].north_shifts)),
            east_shifts=num.concatenate(
                (dss[0].east_shifts, dss[1].east_shifts)),
            times=num.concatenate((dss[0].times, dss[1].times)),
            depths=num.concatenate((dss[0].depths, dss[1].depths)),
            m6s=num.vstack((dss[0].m6s, dss[1].m6s)))
    ...
    def get_magnitude(self, store=None, target=None):
        ...
        return pmt.moment_to_magnitude(num.sum(
            [pmt.magnitude_to_moment(mag) for mag in (mag1, mag2)]))
